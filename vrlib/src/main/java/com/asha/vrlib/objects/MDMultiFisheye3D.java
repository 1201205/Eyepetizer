...
public class MDMultiFisheye3D extends MDAbsObject3D {
    ...
    private static void generateSphere(float radius, int rings, int sectors, MDAbsObject3D object3D) {
        ...
        for (r = 0; r < rings + 1; r++) {
            for (s = 0; s < sectors + 1; s++) {
                ...
                if (t * 2 < numPoint) {
                    ...
                    float a = (float) (Math.sin(2 * PI * s * S) * (1 - r * R) * 2 * 0.65f) * 0.5f +
                        0.5f;
                    float b = (float) (Math.cos(2 * PI * s * S) * (1 - r * R) * 2 * 0.65f) * 0.5f +
                        0.5f;
        ...
        for (int k = 0; k < numPoint; k++) {
            ...
            Log.e(TAG, String.format("v %d, x=%f y=%f z=%f", k, vertexs[k * 3], vertexs[k * 3 + 1],
                vertexs[k * 3 + 2]));
