...
public class VRUtil {
    ...
    public static void sensorRotationVector2Matrix(SensorEvent event, int rotation, float[] output) {
        ...
        switch (rotation) {
            ...
            case Surface.ROTATION_90:
                ...
                SensorManager.remapCoordinateSystem(mTmp, SensorManager.AXIS_Y,
                    SensorManager.AXIS_MINUS_X, output);
                break;
            case Surface.ROTATION_270:
                SensorManager.getRotationMatrixFromVector(mTmp, values);
                SensorManager.remapCoordinateSystem(mTmp, SensorManager.AXIS_MINUS_Y,
                    SensorManager.AXIS_X, output);
                break;
        }
    ...
    public static void barrelDistortion(double paramA, double paramB, double paramC, PointF src) {
        ...
        double srcR =
            (paramA * dstR * dstR * dstR + paramB * dstR * dstR + paramC * dstR + paramD) * dstR;
    ...
    public static MDVector3D vec3Sub(MDVector3D v1, MDVector3D v2) {
        return new MDVector3D().setX(v1.getX() - v2.getX())
            .setY(v1.getY() - v2.getY())
            .setZ(v1.getZ() - v2.getZ());
    }
