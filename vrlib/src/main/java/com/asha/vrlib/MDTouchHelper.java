            ...
            new GestureDetector.SimpleOnGestureListener() {
                ...
                public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
                    ...
                    if (mAdvanceGestureListener != null) {
                        mAdvanceGestureListener.onDrag(distanceX / mGlobalScale,
                            distanceY / mGlobalScale);
                    }
                    return true;
                }
            });
    ...
    public boolean handleTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (mCurrentMode == MODE_PINCH && event.getPointerCount() > 1) {
                float distance = calDistance(event.getX(0), event.getY(0), event.getX(1),
                    event.getY(1));
